...
//  вариант решения задачи с помощью локального класса
public class FiguresCompare extends NestedFigures {
    ...
    FiguresCompare(Figure[] figures, Compare_Default fc_New) {
    ...
    public static void main(String[] args) {
        ...
        FiguresCompare fcP = new FiguresCompare(figures);   // один параметр
        ...
        //  наш "герой" - локальный класс, потомок вложенного класса
        class Compare_WithArea extends Compare_Default {
            @Override
            boolean greater_Two(Figure figure1, Figure figure2) {
                return figure1.area() > figure2.area();
            }
        }
        Compare_WithArea fc_Over = new Compare_WithArea();
        //  fc_Over - объект "потомка" локального класса
        //  второй параметр для второго конструктора

        FiguresCompare fcA = new FiguresCompare(figures, fc_Over);  // два параметра
